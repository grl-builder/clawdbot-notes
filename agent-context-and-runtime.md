...
This doc explains what goes into that context and how it flows through the agent runtime.

## What is Pi Agent?

Clawdbot uses [Pi Agent](https://github.com/badlogic/pi-mono) as its agent runtime:

- **Library**: `@mariozechner/pi-coding-agent` (external npm package)
- **Role**: Handles LLM sessions, tool execution, and response streaming
- **Wrapper**: Clawdbot wraps it in `pi-embedded-runner` to integrate with sessions and channels

Pi Agent manages the agentic loop - sending prompts to the LLM, parsing tool calls, executing tools, and continuing until the agent produces a final response.
...
```
[Telegram From YourName 2024-01-15 10:30:00] Hey, can you help with this code?
```
...
Workspace context files that customize the agent's behavior:
...
These files are read from your workspace and injected into the system prompt. They're truncated if too large (70% head + 20% tail, max 20k chars).
...
1. **Context Assembled** - System prompt, history, bootstrap files, your message
...
2. **Context assembly** gathers system prompt, history, bootstrap files
